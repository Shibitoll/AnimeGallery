...
const AnimeCard = ({ 
  id, mal_id, malId, title, poster, rating, user_rating, userRating, year, episodes, 
  ...
  // Уніфікуємо пропси
  const actualMalId = mal_id || malId || id;
  ...
  // Локальні стани для миттєвої реакції кнопок (Optimistic UI)
  ...
  // 🛡 ОФІЦІЙНИЙ ПАТЕРН REACT ДЛЯ СИНХРОНІЗАЦІЇ
  const [prevProps, setPrevProps] = useState({
    fav: actualFavorite,
    watchng: actualWatching,
    watch: actualWatched,
    plan: actualPlanned,
  ...
  if (
    actualFavorite !== prevProps.fav ||
    actualWatching !== prevProps.watchng ||
    actualWatched !== prevProps.watch ||
    actualPlanned !== prevProps.plan ||
  ...
  const getAnimePayload = () => ({
    id, mal_id: actualMalId, title, poster, rating, year, episodes, 
    studio: studio || 'Невідомо', genres: displayGenres,
    is_favorite: optFavorite, is_watching: optWatching, in_watchlist: optWatched, planned: optPlanned
  });
  ...
  const handlePrefetch = () => {
    fetch(`http://localhost:8000/api/streaming/info/${actualMalId}/`).catch(() => {});
  };

  return (
    <Card 
      className={`anime-card-modern compact ${optWatched ? 'watched' : ''}`}
      onMouseEnter={handlePrefetch}
    >
      <div className="poster-container">
        <Link to={`/anime/${actualMalId}`} className="poster-link">
      ...
      <div className="card-info compact-info">
        <Link to={`/anime/${actualMalId}`} className="title-link">
        ...
        <div className="card-actions compact-actions">
          ...
          {/* Стан 1: Не дивиться і не переглянуто */}
          ...
          {/* Стан 2: Зараз дивиться (Кнопка "Дивлюся" стала не клікабельною) */}
          {optWatching && !optWatched && (
          ...
          {optWatched && (
            <Button 
              variant="primary" 
              onClick={handleToggleWatched}
              className="full-width-btn watched-btn"
              style={{ marginBottom: '8px', backgroundColor: '#10b981', borderColor: '#10b981' }}
            >
              ✅ Переглянуто
            </Button>
          )}
          ...
          <div className="user-rating-box compact-rating">
            {isEditing ? (
              ...
              <div className="display-rating-wrapper" onClick={() => setIsEditing(true)}>
                ...
                <strong className={`user-score ${parseFloat(optRating) > 0 ? 'rated' : ''}`}>
                  {optRating}
                </strong>
